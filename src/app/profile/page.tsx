...
export default function ProfilePage() {
  ...
  const roleLabel = (session?.user as { role?: string })?.role || 'Lead Operations Engineer';
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-md border border-border-subtle bg-[#F8FAF8] p-3.5 flex items-start gap-3 text-xs text-foreground-secondary">
            ...
            <div className="leading-relaxed">
              <span className="font-semibold text-foreground">Operator Session Identity:</span> Profile parameters are synchronized with your active authentication session and central SCADA directory. To request role elevation or asset assignment transfers, submit an operational change request to the Grid Interconnect Coordinator.
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card className="h-fit">
              <CardHeader className="text-center pb-4 border-b border-border-subtle">
                ...
                <CardDescription className="text-xs break-all">{userEmail || 'Operator Session'}</CardDescription>
              ...
              <CardContent className="pt-4 space-y-3 text-xs">
                <div className="flex justify-between py-1.5 border-b border-border-subtle">
                  ...
                  <span className="font-mono font-semibold text-foreground">{roleLabel}</span>
                </div>
                <div className="flex justify-between py-1.5 border-b border-border-subtle">
              ...
              <CardFooter className="pt-2">
                <Button 
                  variant="outline" 
                  className="w-full text-xs text-critical hover:bg-critical/10 hover:text-critical border-border-subtle min-h-[44px]"
                  onClick={handleSignOut}
                >
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription>
                    Cryptographic token verification and database session attributes
                  </CardDescription>
                </CardHeader>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="p-3 rounded-md border border-border-subtle bg-[#F8FAF8] space-y-1">
                      ...
                      <div className="text-xs font-semibold text-foreground flex items-center gap-2">
                        ...
                        <span className="font-mono">{authProvider}</span>
                      </div>
                    </div>
                  ...
                  <div className="p-4 rounded-md border border-border-subtle bg-[#F8FAF8] space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-foreground font-display">
                          {plant?.name || 'Assigned Renewable Facility'}
                        </span>
                      </div>
                      <span className="font-mono text-xs px-2 py-0.5 rounded-sm bg-white border border-border-subtle font-semibold text-primary-dark">
                        {configuration?.gridNode || 'GRID-INTERCONNECT-01'}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2 text-xs">
                      <div>
                        ...
                        <div className="font-mono font-bold text-foreground">{(configuration?.acCapacityMw ?? 42.0).toFixed(1)} MW AC</div>
                      </div>
                      <div>
                        <div className="text-[11px] text-muted">DC Capacity</div>
                        <div className="font-mono font-bold text-foreground">{(configuration?.dcCapacityMw ?? 50.0).toFixed(1)} MW DC</div>
                      </div>
                      <div>
                        <div className="text-[11px] text-muted">Interconnection</div>
                        <div className="font-mono font-semibold text-foreground">{configuration?.gridOperator || 'GETCO'} {configuration?.gridVoltageKv || 220}kV</div>
                      </div>
                      <div>
                        <div className="text-[11px] text-muted">Generation Source</div>
                        <div className="font-mono font-semibold text-primary-dark">PHYSICS BASELINE</div>
                      </div>
                    </div>

                    <div className="flex items-center gap-2 text-[11px] text-foreground-secondary pt-2 border-t border-border-subtle">
                      <MapPin className="size-3.5 text-primary shrink-0" />
                      <span>{plant ? `${plant.city}, ${plant.state} (${plant.country})` : 'Location Configured'}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
